...
public class TFileDataTable extends JTable implements TEventListener {
    ...
    private final static Logger LOGGER = Logger.getLogger(TFileDataTable.class
            .getName());

    public TFileDataTable(int ccId, TFileDataModel fileDataModel) {
        super(fileDataModel);
    ...
    private void init() {
        ...
        headerRenderer = new ButtonHeaderRenderer[fileDataModel
                .getColumnCount()];
        for (int i = 0; i < fileDataModel.getColumnCount(); i++) {

            headerRenderer[i] = new ButtonHeaderRenderer(fileDataModel,
                    fileDataModel.getColumnNameAt(i));
            ...
            case TFileProperties.TFILE_PROPERTY_SIZE:
                ...
                FileAttributeCellRenderer renderer = new FileAttributeCellRenderer(
                        style, JLabel.RIGHT);
            ...
            case TFileProperties.TFILE_PROPERTY_MOD_TIME:
                ...
                col.setCellRenderer(new FileAttributeCellRenderer(style));
                break;
            }
        }
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                final int row = TFileDataTable.this.rowAtPoint(e.getPoint());

                if ((e.getClickCount() > 1 && dirOpenWithDouble)
                        || !dirOpenWithDouble) {
                    ...
                    if (row == 0) {
                        TEventMulticaster.getInstance().fireTEvent(
                                TFileDataTable.this, TFileDataTable.this.ccId,
                                new TChdirMessage(".."));
                    } else {
                        TFileProperties fileProp = (TFileProperties) TFileDataTable.this.fileDataModel
                                .getValueAt(row, 0);
                        if (fileProp.isDirectory()) {
                            TEventMulticaster.getInstance().fireTEvent(
                                    TFileDataTable.this,
                                    TFileDataTable.this.ccId,
                                    new TChdirMessage(fileProp
                                            .getAbsoluteFileName()));
                        }
                    }
                }
        ...
        // adjust row height to font size
        int rowHeight = TStyleChooser.getStyle().getTableFont().getSize() + 2 + 2;
    ...
    protected void loadSettings() {
        try {
            dirOpenWithDouble = MainWin.config.getProgramSettings()
                    .getBooleanOption(
    ...
    public TFileProperties getFirstSelectedFileData() {
        ...
        if (getSelectedRowCount() > 0 && getRowCount() > 1) {
            ...
            if (row > 0) {
                back = (TFileProperties) fileDataModel
                        .getFilePropertiesAtRow(row);
            }
        }
        return back;
    }
    ...
    public void restoreSelectedRow() {
        if (lastFocusedRow != -1) {
            ...
            // logger.debug("ccid : " + ccId + " : restore row :" +
            // lastFocusedRow);
        } else {
            setColumnSelectionInterval(0, 0);
            setRowSelectionInterval(0, 0);
    ...
    /*
     * register custom key events with table e.g. End and Home keys
     */
    ...
    private void addKeyAction(String shortcut) {
        ...
        if (ksList != null) {
            for (final KeyStroke ks : ksList) {
                ...
                getActionMap()
                        .put(shortcut, new TableKeyAction(shortcut, this));
            }
        } else {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (cmd.equals("switch-browser")) {
                ...
                if (getSelectedRowCount() > 0) {
                    TEventMulticaster.getInstance()
                            .fireTEvent(
                                    source,
                                    ccId,
                                    new TStandardMessage(
                                            TMessage.ID.COPY_FILES_MESSAGE));
                }
                ...
                if (selected == null) {
                    ...
                    if (lastSelected < getRowCount() - 1) {
                        final int newToSelectAndFocus = lastSelected + 1;
                        addRowSelectionInterval(newToSelectAndFocus,
                                newToSelectAndFocus);
                    }
                }
                ...
                TEventMulticaster.getInstance().fireTEvent(TFileDataTable.this,
                        ccId,
                ...
                TEventMulticaster.getInstance().fireTEvent(TFileDataTable.this,
                        ccId, new TChdirMessage(".."));
            } else if (cmd.equals("end-key")) {
                selectRow(getRowCount() - 1);
            } else if (cmd.equals("home-key")) {
                selectRow(0);
            } else if (cmd.equals("select-dir-sync")) {

                fileDataModel.toggleRowIdForSync(getSelectedRow());
                updateUI();
    ...
    class HeaderListener extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent e) {
            ...
            TFileDataTable.this.clearSelection();
            ...
            r.setBounds(offset + colXOffset, offset, cw - 2 * offset, ch - 2
                    * offset);
            if (!r.contains(p)) {
                return;
            }
            ...
            header.repaint();
            TEventMulticaster emc = TEventMulticaster.getInstance();
            emc.fireTEvent(TFileDataTable.this, TFileDataTable.this.ccId,
