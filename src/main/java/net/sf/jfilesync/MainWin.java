...
public class MainWin extends JFrame implements ActionListener, ComponentListener, TEventListener, GWorkerListener,
  ...
  private SyncMethod[] methods = { new TimeSyncMethod(), new Sha1SyncMethod(), new Md5SyncMethod(),
      new SizeSyncMethod() };
  ...
  private final static Logger LOGGER = Logger.getLogger(MainWin.class.getName());
  ...
  public MainWin() {
    ...
    TControlCenterFactory.getControlCenterBus().fireControlCenterBusEvent(new ControlChangeEvent(this, 0));
  }
  ...
  private void initUI() {
    ...
    JButton copyButton = new JButton(LanguageBundle.getInstance().getMessage("label.copy"), new ImageIcon(getClass()
        .getResource("/net/sf/jfilesync/gui/themes/default/save_all.png")));
    copyButton.setActionCommand("copy_command");
    copyButton.addActionListener(this);
    copyButton.setToolTipText(LanguageBundle.getInstance().getMessage("label.copy"));
    ...
    JButton syncButton = new JButton(LanguageBundle.getInstance().getMessage("label.sync"), new ImageIcon(getClass()
        .getResource("/net/sf/jfilesync/gui/themes/default/view_right.png")));
    syncButton.addActionListener(this);
    syncButton.setToolTipText(LanguageBundle.getInstance().getMessage("label.sync"));
    ...
    final String title = ConfigDefinitions.PROGRAM_NAME + "@" + TMiscTool.getLocalHostName() + " "
        + ConfigDefinitions.PROGRAM_VERSION;

    setTitle(title);

    setIconImage(TImageIconProvider.getInstance().getImage(TImageIconProvider.FRAME_ICON));

    /*
     * - a way to avoid redundant use of ActionListeners and KeyListeners on
     * JButton - without this change of the InputMap hitting Enter will not fire
     * an ActionEvent even though the JButton has the focus
     */
  ...
  private void setupMenuBar(JMenuBar menubar) {
    ...
    jMenuItem1.setText(LanguageBundle.getInstance().getMessage("menu.exit"));
    ...
    optionItem.setText(LanguageBundle.getInstance().getMessage("menu.options"));
    ...
    aboutItem.setText(LanguageBundle.getInstance().getMessage("menu.about"));
    aboutItem.setActionCommand("aboutdiag");
    aboutItem.addActionListener(this);
    JMenuItem changelogItem = new JMenuItem();
    changelogItem.setText(LanguageBundle.getInstance().getMessage("menu.changelog"));
  ...
  protected void loadSettings() {
    try {
      final String userLang = config.getProgramSettings().getStringOption(TProgramSettings.OPTION_USER_LANG);

      LanguageBundle.setLanguage(userLang);

      guru = config.getProgramSettings().getBooleanOption(TProgramSettings.OPTION_GURU_MODE);
  ...
  private void showChangeLog() {
    try {
      final String changesText = TextFileReader.readFileContentsFromResource("/net/sf/jfilesync/Changes.txt");
      new TextViewFrame(changesText, LanguageBundle.getInstance().getMessage("menu.changelog")).setVisible(true);
    } catch (IOException e) {
  ...
  private void closingEvent() {
    ...
    try {
      showPrompt = !config.getProgramSettings().getBooleanOption(TProgramSettings.OPTION_DO_NOT_PROMPT_FOR_EXIT);
    } catch (SettingsTypeException ste) {
    ...
    if (showPrompt) {
      ...
      if (option == CapivaraCloseDialog.OK_OPTION) {
        if (ccd.isNeverPromptSelected()) {
          try {
            config.getProgramSettings().setProgramOption(TProgramSettings.OPTION_DO_NOT_PROMPT_FOR_EXIT,
  ...
  private void sync() throws IOException {
    ...
    if (con1 == null || con2 == null || !con1.isConnected() || !con2.isConnected()) {
      TErrorHandling.reportError(TErrorHandling.ERROR_CONNECTION_NOT_ACTIVE);
      return;
    }

    final List<SyncMethod> syncMethodsAvailable = new ArrayList<SyncMethod>();
    for (int i = 0; i < methods.length; i++) {
      if (methods[i] != null && methods[i].isApplicable(con1.getPlugin(), con2.getPlugin())) {
        syncMethodsAvailable.add(methods[i]);
      }
    }
    if (syncMethodsAvailable.size() > 0) {

      SyncMethod[] methodsToUse = (SyncMethod[]) syncMethodsAvailable.toArray(new SyncMethod[syncMethodsAvailable
          .size()]);
      ...
      if (!inExpertMode() && (!con1.isLocalConnection() || !con2.isLocalConnection())) {
      ...
      SyncOptionDialog sod = new SyncOptionDialog(this, lc.getConnectionDetails(), rc.getConnectionDetails(),
      ...
      ConcurrentSyncTreeBuilder myWorker = new ConcurrentSyncTreeBuilder(this, syncSettings.getSyncID(), con1,
          lc.getCurrentPath(), lc.getConnectionDetails(), con2, rc.getCurrentPath(), rc.getConnectionDetails());
  ...
  private boolean inExpertMode() {
    boolean ret = false;
    try {
      ret = config.getProgramSettings().getBooleanOption(TProgramSettings.OPTION_EXPERT_MODE);
    } catch (SettingsTypeException ste) {
      LOGGER.warning(ste.getMessage());
    }
    return ret;
  ...
  private void copyData(final CopySettings cs) {
    ...
    final AbstractConnectionProxy connection1 = cs.getSource().getConnection();
    final AbstractConnectionProxy connection2 = cs.getTarget().getConnection();

    if (connection1 == null || connection2 == null || !connection1.isConnected() || !connection2.isConnected()) {
      TErrorHandling.reportError(TErrorHandling.ERROR_CONNECTION_NOT_ACTIVE);
      return;
    } else {
    ...
    if (data.length <= 0) {
      ...
      ccd = new CopyConfirmDialog(this, cd1, cd2, data[0].getFileName(), data[0].isDirectory());
    } else {
    ...
    if (ccd != null) {
      ...
      if (copyStruct.getTarget().getConnection().getPlugin().isProvided(GeneralPlugin.PROVIDES_PERMISSION_HANDLING)) {
    ...
    try {
      ...
      follow_symlinks = MainWin.config.getProgramSettings().getBooleanOption(
    ...
    if (!guru) {
      ...
      try {
        int dir_perm = MainWin.config.getProgramSettings().getIntegerOption(TProgramSettings.OPTION_PERMISSIONS_DIR);
        int file_perm = MainWin.config.getProgramSettings().getIntegerOption(TProgramSettings.OPTION_PERMISSIONS_FILE);
    ...
    abstractGuiElement = new CollectFilesDialog(this, cs.getSource().getConnectionDetails());
    abstractGuiElement.addTAbstractGUIElementListener(this);

    activeWorker = new DataCollectWorker(connection1, data, true, follow_symlinks);
    activeWorker.addObserver(observer);
  ...
  private boolean isPermissionHandlingSupported(final CopySettings copySettings) {
    boolean ret = false;

    if (copySettings.getSource().getConnection().getPlugin().isProvided(GeneralPlugin.PROVIDES_PERMISSION_HANDLING)
        && copySettings.getTarget().getConnection().getPlugin().isProvided(GeneralPlugin.PROVIDES_PERMISSION_HANDLING)) {
      ret = true;
    }
    return ret;
  }
  ...
  private static void loadConfiguration() {
    ...
    if (opts.hasConfigDir()) {
      CapivaraRuntimeConfiguration.getInstance().setUserConfigDir(opts.getConfigDir());
    }
    ...
    if (configHandler.configDirExists()) {
      ...
      if (configHandler.configFileExists()) {
        ...
        try {
          ...
          int opt = JOptionPane.showConfirmDialog(null,
              "Error while reading configuration file.\nCreate new configuration?", "Configuration read error",
      ...
      if (!opts.hasConfigDir()) {
        ...
        final String userDir = CapivaraRuntimeConfiguration.getInstance().getUserConfigDir();

        final File userDirFile = new File(userDir);
        if (!userDirFile.exists()) {
          TErrorHandling.reportError(TErrorHandling.ERROR_IO_GENERAL, userDir + " does not exists");
          System.exit(-1);
        }

        String question = "Create new configuration directory in %s?";
        question = question.replace("%s", CapivaraRuntimeConfiguration.getInstance().getUserConfigDir());

        final int opt = JOptionPane.showConfirmDialog(null, question, "Confirmation", JOptionPane.ERROR_MESSAGE,
      ...
      if (createNewConfig) {
        ...
        TErrorHandling.reportError(TErrorHandling.ERROR_IO_GENERAL, "No configuration! Exiting...");
        System.exit(-1);
      }
  ...
  private static void saveConfiguration() {
    ...
    if (config != null) {
      writer.writeConfig(config, ConfigFileHandler.getInstance().getConfigFileLocation());
    }
  }
  ...
  private static void setupConfiguration() {
    String lang = null;
    try {
      lang = config.getProgramSettings().getStringOption(TProgramSettings.OPTION_USER_LANG);
    } catch (SettingsTypeException ste) {
  ...
  private static void loadKeyBindings() {
    ...
    try {
      ...
      KeyStrokeConfigLoader loader = new KeyStrokeConfigLoader(instream, instream2);

      loader.load();
      keyMap = loader.getKeyStrokeModel();

    } catch (IOException e) {
      LOGGER.log(Level.SEVERE, "Error while loading keymap file!\n" + e.getMessage());
      JOptionPane.showMessageDialog(null, "Keybindings not loaded. exiting");
      System.exit(-1);
    }
  }
  ...
  public void processEvent(TEvent e) {
    ...
    case CONTROLCENTER_CHANGE_MESSAGE:
      ...
      ControlCenterSwitchMessage ccsm = (ControlCenterSwitchMessage) e.getMessage();
    ...
    case SAVE_CONFIG_MESSAGE:
      ...
      break;
    default:
      LOGGER.warning("Unhandled message type: " + e.getMessage().getMessageType());
      break;
    }
  ...
  public void workerDone(GWorkerEvent e) {
    ...
    if (e.getSource() instanceof ConcurrentSyncTreeBuilder) {
      SyncTreesMessage msg = (SyncTreesMessage) ((GWorker) e.getSource()).construct();
      ...
      int syncID = ((ConcurrentSyncTreeBuilder) e.getSource()).getSyncID();

      activeWorker = new SyncComparator(syncID, leftBrowser.getFocusedControlCenter().getConnection(), tree1,
          rightBrowser.getFocusedControlCenter().getConnection(), tree2, chosenMethod);
      activeWorker.addObserver(observer);
      ...
      SyncDiffEvent sde = (SyncDiffEvent) ((GWorker) e.getSource()).construct();
      ...
      SyncTreeExplorerFrame syncFrame = new SyncTreeExplorerFrame(this, ((SyncComparator) e.getSource()).getSyncID(),
          tree1, diffTree1, tree2, diffTree2, lc.getConnection(), lc.getConnectionDetails(), rc.getConnection(),
      ...
      SyncSettingsStore.getInstance().removeSyncSettings(comparator.getSyncID());
    } else if (e.getSource() instanceof DataCollectWorker) {

      abstractGuiElement.enableGUIElement(false);

      TFileData collection1;
      if (activeWorker != null && activeWorker.getWorkerState() == GWorker.STATE_DONE) {
        collection1 = ((TFileListEvent) activeWorker.construct()).getFileData();
      } else {
        return;
      }
      ...
      FileTransfer2 transfer = new FileTransfer2(this, copyStruct.getSource().getConnection(), copyStruct.getSource()
          .getCurrentPath(), copyStruct.getTarget().getConnection(), copyStruct.getTarget().getCurrentPath(),
      ...
      if (transfer.wasCancelled()) {
        ...
        if (!copyStruct.getSource().getConnection().isConnected()) {
          TErrorHandling.reportError(TErrorHandling.ERROR_CONNECTION_LOST, copyStruct.getSource()
              .getConnectionDetails().toString());
          // copyStruct.getSource().clearCenter();
          SwingUtilities.invokeLater(new ControlCenterClearer(copyStruct.getSource()));
        }
        if (!copyStruct.getTarget().getConnection().isConnected()) {
          targetStateOK = false;
          TErrorHandling.reportError(TErrorHandling.ERROR_CONNECTION_LOST, copyStruct.getTarget()
              .getConnectionDetails().toString());
          // copyStruct.getTarget().clearCenter();
          SwingUtilities.invokeLater(new ControlCenterClearer(copyStruct.getTarget()));
        }
      }
      ...
      TErrorHandling.reportError(TErrorHandling.ERROR_UNKNOWN_WORKER_ERROR);
    }
  ...
  public void workerCancelled(GWorkerEvent e) {
    ...
    if (e.getSource() instanceof SyncComparator || e.getSource() instanceof ConcurrentSyncTreeBuilder) {
  ...
  public static void printJvmConfig() {
    final String[] properties = { "java.version", "java.vm.version", "java.vm.name", "file.separator", "os.name",
        "os.arch", "os.version", "user.dir", "user.home", "user.name", "file.encoding" };

    for (int i = 0; i < properties.length; i++) {
      LOGGER.info(properties[i] + " : " + System.getProperty(properties[i]));
    }
  ...
  private void initConnections() {
    if (opts.hasLeftTarget()) {
      ...
      int port = ConnectionPluginManager.getConnectionModelInstance(opts.getLeftProtocol()).getDefaultPort();
      TConnectionData data = new TConnectionData(this, opts.getLeftHost(), port, opts.getLeftUser(), "",
          opts.getLeftProtocol());
      cc.setInitialDirectory(opts.getLeftPath());
      cc.connect(data);
    } else {
      try {
        if (config.getProgramSettings().getBooleanOption(TProgramSettings.OPTION_CONNECT_TO_LOCAL)) {

          final TControlCenter cc = leftBrowser.getFirstControlCenter();

          final TConnectionData data = new TConnectionData(this);

          if (config.getProgramSettings().getBooleanOption(TProgramSettings.OPTION_USE_GNU_FOR_LOCAL)) {

            data.setProtocol(ConnectionPluginManager.GNU_PLUGIN);
          }

          if (config.getProgramSettings().getBooleanOption(TProgramSettings.OPTION_USE_CUSTOM_PATH)) {

            final String customPath = config.getProgramSettings().getStringOption(TProgramSettings.OPTION_CUSTOM_PATH);
    ...
    if (opts.hasRightTarget()) {
      ...
      int port = ConnectionPluginManager.getConnectionModelInstance(opts.getRightProtocol()).getDefaultPort();
      TConnectionData data = new TConnectionData(this, opts.getRightHost(), port, opts.getRightUser(), "",
  ...
  private void handleQuickConnectMessage(QuickConnectMessage message) {
    ...
    if (message.getNewTab()) {
      targetCC = TControlCenterFactory.getInstance(this).getNextControlCenter();
      ...
      if (targetCC == null) {
        targetCC = rightBrowser.getControlCenter(message.getSourceCCId());
      }
    }
  ...
  private static void initLogging() {
    ...
    try {
      LogManager.getLogManager().readConfiguration(MainWin.class.getResourceAsStream(logPropResource));
    } catch (IOException e) {

      System.err.println("Cannot initialize logging. Error while reading " + logPropResource);
  ...
  private static void setupLookAndFeel() {
    ...
    try {
      lookAndFeelClass = config.getProgramSettings().getStringOption(TProgramSettings.OPTION_LOOK_AND_FEEL_CLASS);
    } catch (SettingsTypeException e) {
      LOGGER.warning(e.getMessage());
    }
  ...
  private static void setupLanguage() {
    ...
    try {
      SwingUtilities.invokeAndWait(new Runnable() {
        public void run() {
          LanguageDialog langDia = new LanguageDialog(TStartup.getSplashScreenFrame());
          langDia.setVisible(true);
          String userLang = langDia.getSelectedLanguage();
          try {
            config.getProgramSettings().setProgramOption(TProgramSettings.OPTION_USER_LANG, userLang);
          } catch (SettingsTypeException e) {
            LOGGER.warning("Cannot store language settings" + e.getMessage());
            e.printStackTrace();
          }
        }
      });
  ...
  private static void showNoticeDialog(final JFrame w) {
    try {
      boolean showNote = config.getProgramSettings().getBooleanOption(TProgramSettings.OPTION_SHOW_NOTE);
      if( showNote ) {
        SwingUtilities.invokeLater(new ComponentVisibleRunner(new InfoDialog(w), true));
      }
    } catch(IOException e) {
